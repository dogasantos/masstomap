    ...
    for fname in sorted(files):
        if ".nmap.grepable." in fname:
            ...
            for line in contents:
                line_clean = str(bytes(re.sub('\#\sNmap\sdone\sat\s.*\n#\sNmap\s\d\.\d\d\sscan\sinitiated\s.*\n', '',line)),'UTF-8', errors='strict')
                grepable_final_report.write(line_clean.encode('UTF-8', errors='strict'))
            gp.close()
            if verbose:
        ...
        if ".nmap.text." in fname:
            ...
            for line in contents:
                line_clean = str(bytes(re.sub('Service detection.*?\n\#\sNmap\sDone\sat\s.*?\n\#\sNmap\s\d\.\d\d\sscan\sinitiated\s.*$','',line)), 'UTF-8', errors='strict')
                text_final_report.write(line_clean.encode('UTF-8', errors='strict'))
            tf.close()
            if verbose:
                print("  + Removing: %s" % str(fname))
            os.unlink(fname)
